...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if(len(formals) != len(vals)):
            raise SchemeError
        formalsfirst = formals
        formalsrest = formals
        valsfirst = vals
        valsrest = vals
        while(formalsrest):
            formalsfirst, formalsrest = formalsrest.first, formalsrest.second
            valsfirst, valsrest = valsrest.first, valsrest.second
            child.define(formalsfirst, valsfirst)
        # END PROBLEM 11
        return child
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            return self.fn(*[arg for arg in python_args])
        except TypeError:
            raise SchemeError
        # END PROBLEM 4
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    bind_names, bind_values = nil, nil
    while bindings:
        check_form(bindings.first, 2, 2)
        bind_names = Pair(bindings.first.first, bind_names)
        bind_values = Pair(scheme_eval(bindings.first.second.first, env), bind_values)
        bindings = bindings.second
    check_formals(bind_names)
    child_env = env.make_child_frame(bind_names, bind_values)
    return child_env
    # END PROBLEM 15
...
def do_mu_form(expressions, env):
    ...
    if expressions.second:
        expressions_body = expressions.second
    else:
        expressions_body = expressions.second.first
    return MuProcedure(formals, expressions_body)
    # END PROBLEM 16
