...
class Ant(Insect):
    ...
    def can_contain(self, otherAnt):
        return self.container and not otherAnt.container and not self.ant
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        """
        # BEGIN Problem 3B
        # while loop of BEE through ENTRANCES
            # while.self.place.entrance?
        thisPlace = self.place
        counter = 0
        while (thisPlace != hive):
            if len(thisPlace.bees) > 0 and counter<=self.max_range and counter>=self.min_range:
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        ...
        if self.digesting > 0:
            ...
            while self.place.bees != [] and self.digesting == 0:
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def __init__(self):
        # BEGIN Problem 9
        self.armor = 1
        self.is_queen = not QueenAnt.num_of_queens
    ...
    def action(self, colony):
        ...
        if not self.is_queen:
            ...
            p1 = self.place.exit
            while p1:
                if p1.ant and not p1.ant.doubled:
                    p1.ant.damage *= 2
                    p1.ant.doubled = True
                if p1.ant and p1.ant.container and p1.ant.ant and not p1.ant.ant.doubled:
                    p1.ant.ant.damage *= 2
                    p1.ant.ant.doubled = True
                p1 = p1.exit
            self.throw_at(self.nearest_bee(colony.hive))
        # END Problem 9
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 9
        self.armor -= amount
        if self.armor == 0 and self.is_queen == True:
            bees_win()
        elif self.armor == 0 and self.is_queen == False:
            self.place.remove_insect(self)
        # END Problem 9
...
def make_stun(action):
    """Return a new action method that does nothing action -- An action method of some Bee
    """
    # BEGIN Problem EC
    def act(duration):
        while duration > 0:
